...
def run():
    ...
    while wss.conn.connected.is_set():
      ...
      for row in rows_to_insert[0]:
        side = "Sell" if row[2] < 0 else "Buy"
        r = {"ts": rows_to_insert[1], "price": row[1],"id": row[0], "side": side, "size": abs(row[2])}
        rows.append(r)
